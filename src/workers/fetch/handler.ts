...
const processFetchMessage = async (message: FetchQueueMessage): Promise<void> => {
  ...
  try {
    await runFetchStep(message.jobId);
...
const runFetchStep = async (jobId: string): Promise<void> => {
  // TODO: Replace with real Gmail fetch + S3 upload implementation.
  // Keep this function idempotent using jobId as storage key prefix.
  void jobId;
};
